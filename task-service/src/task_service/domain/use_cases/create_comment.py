...
class CreateCommentUseCase:
    """Use case для добавления комментария к задаче."""
    ...
    @log(logger)
    async def execute(self, task_id: int, data: CommentCreate) -> CommentSchema:
        async with self._database.session() as session:
            # Проверяем что задача существует
            await self._task_repository.get_one_task(session, task_id)
            return await self._comment_repository.create_comment(session, task_id, data)